...
/* Keys to input */
const unsigned char test_msg1[3] = { 'A', 'B', 'C' };
const unsigned char test_msg2[4] = { 's', 'p', 'a', 'm' };
const unsigned char test_msg3[7] = { 'a', 'r', 'c', 'f', 'o', 'u', 'r' };

/* Test vectors */
/* Key 'ABC' stream test vectors */
const unsigned char test_strm1[32] =
...
const unsigned char test_strm2[32] =
...
const unsigned char test_strm3[32] =
...
void loop() {
  ...
  sc.setup(&s_ctx, test_msg1, sizeof(test_msg1));
  ...
  for (size_t i = 0; i < sizeof(buf); i++) {
    ...
    if (buf[i] != test_strm1[i]) {
      Serial.println("** WARNING: Output_1 != Test_Vectors **");
    }
  }

  /* Key: spam */
  Serial.println("stream('spam'):");
  sc.setup(&s_ctx, test_msg2, sizeof(test_msg2));
  for (unsigned int i = 0; i < sizeof(buf); i++) {
    buf[i] = sc.stream(&s_ctx);
    if (buf[i] < 0x10) {
      Serial.write('0');
  ...
  for (size_t i = 0; i < sizeof(buf); i++) {
    /* If the output is wrong */
    if (buf[i] != test_strm2[i]) {
      Serial.println("** WARNING: Output_2 != Test_Vectors **");
    }
  }

  /* Key: arcfour */
  Serial.println("stream('arcfour'):");
  sc.setup(&s_ctx, test_msg3, sizeof(test_msg3));
  for (unsigned int i = 0; i < sizeof(buf); i++) {
    buf[i] = sc.stream(&s_ctx);
    if (buf[i] < 0x10) {
      Serial.write('0');
  ...
  for (size_t i = 0; i < sizeof(buf); i++) {
    /* If the output is wrong */
    if (buf[i] != test_strm3[i]) {
      Serial.println("** WARNING: Output_3 != Test_Vectors **");
    }
  }
